...
function isMarkdownFile(filePath) { // verifica si un archivo tiene una extensión de archivo compatible con Markdown
  const markdownExtensions = ['.md', '.mkd', '.mdwn', '.mdown', '.mdtxt', '.mdtext', '.markdown', '.text'];
  const ext = path.extname(filePath).toLowerCase(); // se obtiene la extensión del archivo utilizando el método path.extname y se convierte a minúsculas
